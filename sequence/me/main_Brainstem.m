...
RFspoil = true;
nx = 104; ny = nx; nz = 44;
voxelSize = [1.7 1.7 4] * 1e-3;   % m
alpha = 90;
pf_ky = 0.75; %(nx-3*6)/nx;

TR = 2.4;  %0.1*11;   % volume TR (sec)1.7
PNSwt = [0.8 1 0.7];   % PNS direction/channel weights

mb = 2; Ry = 2; caipiShiftZ = 1;
...
rows = IZ(1:40);
cols = IY(1:40);
idx = sub2ind(size(mask),rows,cols);
mask(idx) = 1;
figure;im(mask')
...
switch lower(vendor)
    case 'ge'
        ...
        entryFileNumber = 4500;  % starting value
...
if TODO(1)
    fn = 'fmri';
    nFramesTmp = 4;   % since number 'runs' (opnex) in pge2 is limited
